...
#
# `role_arn` is intentionally injected via TFC workspace env var
# TENCENTCLOUD_ASSUME_ROLE_ARN so each peer workspace can point to its own
# role without code change.
# ============================================================================
provider "tencentcloud" {
  ...
  assume_role_with_web_identity {
    ...
    # Static value avoids whitespace/empty-segment failures. Audit traceability
    # is preserved by CloudAudit which records source IP + JWT sub anyway.
    session_name       = "tfc_sph_iac_demo"
